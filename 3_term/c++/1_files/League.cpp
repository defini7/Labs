...
bool League::Load(const std::string_view filename)
{
    std::ifstream is(filename.data());
    if (!is) return false;

    is >> m_Title >> m_MatchesCount;
...
bool League::Save(const std::string_view filename)
{
    std::ofstream os(filename.data());
    if (!os) return false;

    os << m_Title << std::endl;
    os << m_MatchesCount << std::endl;

    for (size_t i = 0; i < m_MatchesCount; i++)
    {
        Match& m = m_Matches[i];
        os << m.Time.ToInt() << '|' << m.Teams[0] << '|' << m.Teams[1] << std::endl;
    }
